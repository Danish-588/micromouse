...
void pwm_config(struct pwm_pins *pin, float duty, float freq,int perc)
{
	...
	if((pin->timer == TIM2)||(pin->timer == TIM3)||(pin->timer == TIM4)||(pin->timer == TIM5))
	{
		temp_freq = round(90000000.0/freq);
		if((temp_freq>65536)&&((pin->timer == TIM3)||(pin->timer == TIM4)))
		{
