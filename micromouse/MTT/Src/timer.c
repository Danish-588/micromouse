...
timer14 = {TIM4,&isr_timer14,emptyfunc};
...
void timer_config(struct isr_timer *timer, float freq)
{
	...
	if((timer->timer == TIM2)||(timer->timer == TIM3)||(timer->timer == TIM4)||(timer->timer == TIM5))
	{
		temp_freq = round(90000000.0/freq);
		if((temp_freq>65536)&&((timer->timer == TIM3)||(timer->timer == TIM4)))
		{
